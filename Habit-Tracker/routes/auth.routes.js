...
router.post("/login", isLoggedOut, (req, res, next) => {
  ...
  if (username === "" || password === "") {
    res.status(400).render("auth/login", {
      errorMessage:
        "All fields are mandatory. Please provide username, email and password.", layout: '/layouts/layout2'});
  ...
  if (password.length < 6) {
    return res.status(400).render("auth/login", {
      errorMessage: "Your password needs to be at least 6 characters long.", layout: '/layouts/layout2'});
  }
  ...
  User.findOne({ username })
    .then((user) => {
      ...
      if (!user) {
        res
          .status(400)
          .render("auth/login", { errorMessage: "Wrong credentials.", layout: '/layouts/layout2'});
        return;
      }
...
router.get("/logout", isLoggedIn, (req, res) => {
  req.session.destroy((err) => {
    if (err) {
      res.status(500).render("auth/logout", {errorMessage: err.message, layout: '/layouts/layout2'});
      return;
    }
